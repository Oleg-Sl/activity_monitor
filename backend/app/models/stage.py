...
class Stages(Base):
    ...
    entity_id: Mapped[str | None] = mapped_column(String(50))           # кодовый идентификатор сущности и стадии (DYNAMIC_166_STAGE_31 и т.д.)
    status_id: Mapped[str] = mapped_column(String(50))                  # кодовый идентификатор стадии (DT166_31:NEW и т.д.)
    category_id: Mapped[int | None]                                     # числовой идентификатор стадии
    name: Mapped[str] = mapped_column(String(100))                      # название стадии
    name_init: Mapped[str | None] = mapped_column(String(100), server_default="")  # обобщенное название группы стадий 
    semantic: Mapped[str | None] = mapped_column(String(5))             # семантичекий код стадии
